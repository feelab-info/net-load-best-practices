...
def load_irradiance_data():
    data = pd.DataFrame()
    for path in ['../../Dataset/PT/solar_cast_2019.csv', '../../Dataset/PT/solar_cast_2020.csv', '../../Dataset/PT/solar_cast_2020_2022.csv']:
        data_temp = pd.read_csv(path)
        data_temp['Date']=pd.to_datetime(data_temp['PeriodEnd'] if 'PeriodEnd' in data_temp.columns else data_temp['period_end'] )
        data_temp=data_temp.drop(['PeriodStart', 'Period',  'PeriodEnd']  if 'PeriodEnd' in data_temp.columns else [ 'period_end', 'period']   , axis=1)
...
def load_all_phases_data():
    ...
    power = pd.DataFrame()
    for path in ['../../Dataset/PT/substation_2018_2021.csv', '../../Dataset/PT/substation_2020_2021.csv', '../../Dataset/PT/substation_all_phases.csv']:
        data_temp = pd.read_csv(path)
