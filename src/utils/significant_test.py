...
def run_statistical_test(ax, real_netload, data, norm=1, test='GW', sample_per_period=48, version='multivariate', cmap = sns.light_palette('#525252', as_cmap=True)):
            ...
            if model1 == model2:
                ...
                if test=='GW':
                    ...
                    p_values.loc[model1, model2] = DM(p_real=real_netload.values.reshape(-1, sample_per_period), 
                                                        ...
                                                        p_pred_2=data.loc[:, model2].values.reshape(-1, sample_per_period), 
                                                        norm=norm, version=version)
    plt.imshow(p_values.astype(float).values, cmap=cmap)
